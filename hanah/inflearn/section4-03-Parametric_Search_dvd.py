...
s = max(lens) # 한 곡이 하나의 DVD에 들어가는 경우에 이 DVD의 용량은 가장 긴 곡의 길이가 되기 때문 
e = sum(lens)
res = 0
...
while s <= e:
    mid = (s + e) // 2
    cnt = 1  # 필요한 DVD의 개수
    sum = 0  # DVD 1개에 들어간 총 녹화 길이
    for x in lens:
        if (sum + x) > mid: # 용량초과 
            cnt += 1 # DVD 1개 추가
            sum = x # sum은 현재 동영상의 길이로 초기화(새 DVD에 들어간 총 녹화 길이)
        else:
            sum += x

    if cnt <= m: # 조건 만족. 더 작은 m이 있는지 찾아야 함 >> 왼쪽으로 탐색
        e = mid - 1
        res = mid
    else: # 조건 불만족. DVD 용량을 늘려야 하므로 오른쪽으로 탐색
        s = mid + 1
